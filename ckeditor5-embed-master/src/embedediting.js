	...
	init() {
		...
		editor.conversion.for( 'downcast' ).elementToElement( {
			...
			view: ( modelElement, viewWriter ) => {
				if( viewWriter ){
					const figure = viewWriter.writer.createContainerElement( 'figure', { class: (enablePlayer?'embed':'embed embed_wrapper') } );
					const iframeElement = viewWriter.writer.createEmptyElement( 'iframe', modelElement.getAttributes() );
					const label = t( 'embed widget' );
					viewWriter.writer.insert( viewWriter.writer.createPositionAt( figure, 0 ), iframeElement );
					return toWidget( figure, viewWriter.writer, { label } );
				}return '';
			}
		} ) ;
		...
		editor.conversion.for( 'upcast' ).elementToElement( {
		    ...
		    model: ( viewElement, modelWriter ) => {
		        return modelWriter.writer.createElement( 'embed', getViewElementAttributes(viewElement) );
		    }
		} );
